...
**Ditulis:** 20 September 2026
**Keadaan repositori saat ditulis:** branch `main`, commit `6361710` (sama dengan `origin/main`)
...
|---|---|
| Berkas Kotlin | 68 |
| Berkas uji unit | 31 berkas, 113 uji, semuanya lulus |
...
2. Periksa berkas itu benar sebelum dipakai:
   ...
   # harus: 5d1549f0edf598a2c2231514b444c45a36e8c4da4f62b6d03008b79be4dce5fd
   ```
...
3. Build: `cd android && ./gradlew assembleDebug`. URL backend sudah ada sebagai default di `android/app/build.gradle.kts` dan dapat ditimpa lewat `gradle.properties` (`SIAGA_BACKEND_BASE_URL`).
...
```bash
cd android && ./gradlew --init-script /path/ke/altbuild.gradle.kts :app:assembleDebug
```
...
**Berkas Figma:** `kFsajBR2QE1XkEFdxR1rzv`, halaman **App**, node `58:2670`. Desain digambar langsung sebagai layar, bukan sebagai pustaka komponen terpisah — atas permintaan Mikail, untuk menghemat waktu dan token. Jangan membongkarnya menjadi komponen kecuali diminta.
...
- **55 ikon Material Symbols Rounded** sebagai vector drawable `ic_ms_*.xml` (Apache 2.0, juga tercatat). Ikon panah diunduh ulang dengan `wght700grad200` karena panah tipis tidak terbaca sambil berjalan.
...
| | |
|---|---|
| Ukuran | 76.419.072 byte (72,9 MiB) |
| SHA-256 | `5d1549f0edf598a2c2231514b444c45a36e8c4da4f62b6d03008b79be4dce5fd` |
| Tabel lama | `tb_nodes`, `tb_edges`, `tb_routes`, `tb_tes`, `tb_inundation_zones`, `tb_safe_zones` |
| Tabel baru | `tb_tea` (12), `tb_tea_routes` (31.813), `tb_tea_next` (63.614) |
| View baru | `v_fasilitas_evakuasi` (143 TES + 12 TEA) |

Dua hal yang sudah diverifikasi dan tidak perlu diulang:
...
1. **Tujuh perapian UI pada commit `6361710`** — kompas, legenda zona, kontras zona, ketuk dua kali peta, animasi, ketebalan panah, popup putih, hitung mundur bergerak. Kode sudah lulus kompilasi dan uji unit, tetapi belum pernah dilihat di layar nyata. **Ini pekerjaan pertama yang harus dilakukan.** Penundaannya disengaja (permintaan Mikail), bukan kelalaian.
...
4. NF-04 pada tiga perangkat lain.
...
- Uji perangkat untuk tujuh perapian UI `6361710`.
- Bersihkan composable lama yang tidak terpakai di `EvacuationScreen.kt` (berkas sudah 3.479 baris).
...
- Muat dataset dengan checksum `5d1549f0…` ke Railway agar laporan jalur terhalang dapat terkirim.
